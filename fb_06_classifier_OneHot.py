...
def init_dialog():
	...
	session_info = {'ParticipantID':'00', 'SessionNumber':'01', 'Version': 6.0}
...
def Acquisition(dataOutQ1, dataOutQ2, saveQ):
	...
	chs_ssvep = [1,2]	#available channels [1 to 8]
	...
	threadLock.acquire()
	saveQ.put([thesamples[:,:], thefilterd[:,:], thetimestamps[:]])
	threadLock.release()
	###
	print('\n\n|||Ending acquisition')
	P300Th.join()
...
def SSVEPfun(dataInQ, featureQ, saveQ):
	...
	while not E.is_set():
		...
		if not E.is_set():
			...
			if not filter1_set.size == 0: filter1_set = np.vstack((filter1_set, f1))
			else: filter1_set = np.array(f1)
			if not filter2_set.size == 0: filter2_set = np.vstack((filter2_set, f2))
			else: filter2_set = np.array(f2)
			if not filter3_set.size == 0: filter3_set = np.vstack((filter3_set, f3))
			else: filter3_set = np.array(f3)
			...
			saveQ.put([filter1_set, filter2_set, filter3_set, feature_set, means_set, label_set])
			threadLock.release()
...
def CCA_classifier_train(featureQ, modelQ, saveQ):
	...
	while not E.is_set():
		...
		if not E.is_set():
			...
			if int(bin_lbl,2) == 1:
				if not print_feat1.size == 0:
					print_feat1 = np.vstack((print_feat1, feat))
				else:
					print_feat1 = np.array(feat)
			elif int(bin_lbl,2) == 2:
				if not print_feat2.size == 0:
					print_feat2 = np.vstack((print_feat2, feat))
				else:
					print_feat2 = np.array(feat)
			elif int(bin_lbl,2) == 4:
				if not print_feat4.size == 0:
					print_feat4 = np.vstack((print_feat4, feat))
				else:
					print_feat4 = np.array(feat)
			...
			#Creating label set for training
			if not label_set.size == 0:
				label_set = np.vstack((label_set, ll))
			else:
				label_set = np.array(ll)
	...
	for train_index, test_index in kf.split(feature_set):
		...
		print(y)
		accs.append( (np.sum(y == y_test))/len(y_test) )
		print("ACC = ", accs[count]*100, "\b%")

		count += 1
	print("Average ACC = ", np.mean(accs)*100, "\b%")
...
def CCA_classifier_predict(featureQ, modelQ, classOutQ, dataclassOutQ):
	...
	while not E.is_set():
		...
		if not E.is_set():
			...
			# _featX = modelQ.transform(feat)
			# print(feat.size, _feat.size)
			# y = modelQ.predict(_feat)
...
if __name__ == '__main__':
	...
	if SaveSSVEPQ.qsize():
		while SaveSSVEPQ.qsize():
			try:
				f1, f2, f3, fff, mean_ss, ll = SaveSSVEPQ.get(0)
			except Empty:
				pass
		print("Size LABELS plot", ll.size)
