...
#send to wich square P300 is showing instead
				if TL == sel:
					EPLabel.set()
				else:
					EPLabel.clear()
				...
				for cyc in range(1, 61, 1): #half a second (60frames @ 120Hz)
					...
					self.win.flip()
					ISI.start(0.0081)# ISI.start(0.01245)
...
def init_dialog():
	...
	global _classioutLDAs
	global TRIALNUM
	global P3NUM

	_currDir = os.path.dirname(os.path.abspath(__file__))
	session_info = {'ParticipantID':'00', 'SessionNumber':'01', 'Version': 7.0}
...
def Acquisition(dataOutQ1, dataOutQ2, saveQ):
	...
	while not E.is_set():#while i<1250:
		...
		if not E.is_set():
			...
			while board.get_board_data_count() < Nyquist:
				time.sleep(0.001)
			data = board.get_board_data()

			##::Save stimuli ending timestamp
			end_stim_ts = data[22,-1]
			...
			int_TL = int(str(int(TL[0])) + str(int(TL[1])) + str(int(TL[2])))
			print(int_TL)
			...
			dataOutQ1.put([filtsamples[_chs_p300,-Nyquist:], tt[-Nyquist:], ll]) 	#data is put in the queue
			dataOutQ2.put([filtsamples[_chs_ssvep,-Nyquist:], tt[-Nyquist:], TL]) 	#data is put in the queue			
...
def SSVEPfun(dataInQ, featureQ, saveQ):
	...
	while not E.is_set():
		...
		if not E.is_set():
			...
			N = int(nyq)	# N = SAMPLE_RATE * DURATION
			yf = fft(mean_ss)
			xf = fftfreq(N, 1/fs)
## i = xf >= 0
## xf[i] and yf[i]
	...
	for train_index, test_index in kf.split(feature_set):
		...
		if CCA_mode:
			for yy in y:
				...
				yy[marg] = 1
				yy[oarg] = 0
				bin_lbl = str(int(yy[0])) + str(int(yy[1])) + str(int(yy[2]))
				new_y.append(int(bin_lbl,2))
			#Conditioning labels
			for ll in y_test:
				bin_lbl = str(int(ll[0])) + str(int(ll[1])) + str(int(ll[2]))
				bin_y_test.append(int(bin_lbl,2))
			##Check
			print(y.T) 		#OneHot predicted
			print(y_test.T)	#vs. OneHot test lbls
		else:
			new_y = y
			bin_y_test = y_test
		##Check
		...
		print("ACC = ", '{:.3f}'.format(accs[count]*100), "\b%")
		count += 1
...
def LDA_classifier_train(featureQ, modelQ):
	...
	##Validation
	accs, accs_mean = validate_classifier(feature_set, label_set, 0, 5)
...
def CCA_classifier_train(featureQ, modelQ, saveQ):
	...
	##Validation
	accs, accs_mean = validate_classifier(feature_set, label_set, 1, 5)
...
def CCA_classifier_predict(featureQ, modelQ, classOutQ, dataclassOutQ):
	...
	while not E.is_set():
		...
		if not E.is_set():
			...
			break
###Maybe send each individual sampl
	threadLock.acquire()
	dataclassOutQ.put([feature_set, label_set, y_set])
	threadLock.release()
...
### SAVE LABELS!!
		...
		'Empty0', 'Empty1', 'Empty2', 'Empty3', 'Empty4', 'Empty5', 'Empty6', 'Empty7', 'Empty8', 'Empty9', 'Timestamps']
		#Zero P300_ts column
		df['P300_ts'] = -1
		df['P300_labels'] = -1
		df['SSVEP_labels'] = -1
		...
		for cl in df['Timestamps']:
			for t0, t1, ll, TL in zip(tss[:,0], tss[:,1], tss[:,2], tss[:,3]):
				...
				if '{:.6f}'.format(cl) == '{:.6f}'.format(t1):
					df.loc[df['Timestamps'] == cl, 'P300_ts'] = 1
					# print(cl, t1)
