...
- Saving P300 position (`pl`)
- Inverted TL binary arrays' order
- Fixing graph show at the end
...
class FlickeringBoxes: #using monitor @ 120Hz = 12, 15, 20 or 6, 10, 15
	...
	def start(self):
		...
		# sq_colors = ['#000000', '#FFFFFF']
		# sq_colors = ['#000000', '#B80117']
...
#-------------------##SSVEP stimuli
					if cyc % 3 == 0:#4 == 0:							#for 20Hz use % 3 		#for 15Hz use % 4		#for 30Hz use % 2
						sq0_flag = not sq0_flag
						self.Sq0.setColor(sq_colors[sq0_flag])
					if cyc % 4 == 0:#6 == 0:							#for 15Hz use % 4 		#for 10Hz use % 6		#for 15Hz use % 4
						sq1_flag = not sq1_flag
						self.Sq1.setColor(sq_colors[sq1_flag])
					if cyc % 5 == 0:#10 == 0:							#for 12Hz use % 5 		#for 6Hz use % 10		#for 7.5Hz use % 8
...
def init_dialog():
	...
	session_info = {'ParticipantID':'00', 'SessionNumber':'01', 'Version': 8.3, 'Headset':['','cyton', 'gtec']}
...
def Acquisition(dataOutQ1, dataOutQ2, saveQ):
	...
	while not E.is_set():#while i<1250:
		...
		if not E.is_set():
			...
			avgsamples = scaler.fit_transform(posfilt[:, win+delay:-win].T).T.tolist() #crop padding 2 and normalizing
			# avgsamples = np.squeeze(mne_scaler.fit_transform(posfilt[:, win+delay:-win])).tolist() #crop padding 2 and normalizing
...
##>>>>>>>>>>MODIFY: add actions here to P300 and SSVEP signal before sending it to ...
			...
			tstampsP300.append([ini_stim_ts, end_stim_ts, ll, int_TL, pl])
	...
	print(len(thetimestamps), len(thetimestamps[0]), sep = 'x')
	print(len(thefilttimestamps), len(thefilttimestamps[0]), '\n\n', sep = 'x')
