'''
ON THIS VERSION:
- Script used for testing with real data
- Uses the first set to train the classifiers
- Uses subsequent sets for validation
- Script can be used to test different feature/conditioning methods
'''
...
def SSVEP_pred(the_model, feat_set, label_set, filename):
	pred_set = []
	rgt = 0
	for feat in feat_set:
		y = the_model.predict(feat.reshape(1,-1))
...
if __name__ == '__main__':
	...
	train_feat_set_SSVEP = SSVEPfun(train_flt_data[:, _chs_ssvep, :])
	...
	for i in range(len(valid_flt_data)):
		...
		SSVEP_set = SSVEPfun(_valid_flt[:, _chs_ssvep, :])

		#Classification
		Y_P300 = P300_pred(M_P300_CLASSIFIER, P300_set, valid_P300_lbls[i], valid_P300_pos[i], valid_file_txt[i])
		Y_SSVEP = SSVEP_pred(M_SSVEP_CLASSIFIER, SSVEP_set, valid_SSVEP_lbls[i], valid_file_txt[i])
