...
def Acquisition(dataOutQ1, dataOutQ2, saveQ):
	...
	while not E.is_set():#while i<1250: ###change to global vsar
		P300TRIG.wait(timeout = 1) #function called when
...
def P300fun(dataInQ, featureQ):
	...
	tt = np.array([])
	while not E.is_set():
		# print('End_experiment = ', end_experiment)
		EP300.wait(timeout = 1)
