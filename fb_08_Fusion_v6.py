...
def Acquisition(dataOutQ1, dataOutQ2, saveQ):
	...
	while not E.is_set(): #while still displaying stimuli
		...
		if not E.is_set():
			...
			pf = np.hstack(( np.flip([s[0:win] for s in ss], axis = 1), ss[:][:], np.flip([s[-win:] for s in ss], axis = 1) ))	#create padding  #user np.pad 'reflect' instead?
		...
		if SaveQ.qsize():
			...
			with open(csvsnippet_path__, 'w', newline = "") as f:
				...
				for i in range(np.array(x).shape[0]): 			#number of snippets
					...
					# for k in range (np.array(x).shape[1]):		#number of samples per snippet
					# 	write_vec = []
					# 	# for j in range(np.array(y).shape[1]):	#number of channels
					# 	# 	write_vec.append(y[i][j][k])
					# 	write_vec = y[i][:][k]
					# 	write_vec.append(x[i][k])
					# 	writer.writerow(write_vec)
					# 	progress bar
